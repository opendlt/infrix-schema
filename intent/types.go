...
const (
	...
	// GoalDisclosureApprove satisfies a grant's ApprovalRequired gate through
	// the governed spine, moving it pending -> active and RECORDING who
	// approved it and under which act.
	//
	// It exists because the gate had no door. A grant created with
	// ApprovalRequired:true stays pending until DisclosureRegistry.ApproveGrant
	// runs, and no goal reached that method — so the only way to obtain an
	// active grant was to declare ApprovalRequired:false at creation, which
	// Gap 4-A requires to carry a typed ApprovalWaiverReason, and none of the
	// seven codes honestly describes a disclosure grant. The alternative to
	// this goal was therefore to invent a waiver, which is the exact defect
	// Gap 4-A exists to prevent.
	GoalDisclosureApprove IntentGoalType = "DISCLOSURE_APPROVE"
